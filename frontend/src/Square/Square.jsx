...
const Square = ({ id, gameState, playingAs, currentElement, socket, setGameState, currentPlayer, setCurrentPlayer, setFinishedState, finishedState, finishedArrayState }) => {
  ...
  const onClickSquare = () => {
    ...
    if (!icon) {
      ...
      const mmyCurrentPlayer = currentPlayer;

      socket.emit("playerMoveFromClient", {
        state: {
          id,
          sign: mmyCurrentPlayer
        }
      })
      ...
      setGameState(prevState => {
        ...
        const colIndex = id % 3
        newState[rowIndex][colIndex] = mmyCurrentPlayer;
        return newState;
      })
    }
  }
  ...
  return (
    <div onClick={onClickSquare} className={`square ${finishedState ? 'not-allowed' : ''} ${playingAs !== currentPlayer ? 'not-allowed' : ''} ${finishedArrayState.includes(id) ? finishedState + '-won' : ''}`}>
